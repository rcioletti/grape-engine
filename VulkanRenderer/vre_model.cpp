...
namespace vre {
	...
	VreModel::VreModel(VreDevice& device, const std::vector<Vertex>& vertices) : vreDevice{device}
	{
	...
	void VreModel::createVertexBuffers(const std::vector<Vertex>& vertices)
	{
		...
		void* data;
		vkMapMemory(vreDevice.device(), vertexBufferMemory, 0, bufferSize, 0, &data);
		memcpy(data, vertices.data(), static_cast<size_t>(bufferSize));
		vkUnmapMemory(vreDevice.device(), vertexBufferMemory);
	}
