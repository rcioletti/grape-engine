...
namespace vre {
	...
	VrePipeline::~VrePipeline()
	{
		vkDestroyShaderModule(vreDevice.device(), vertShaderModule, nullptr);
		vkDestroyShaderModule(vreDevice.device(), fragShaderModule, nullptr);
	...
	void VrePipeline::createGraphicsPipeline(const std::string& vertFilepath, const std::string& fragFilepath, const PipelineConfigInfo& configInfo)
	{
		...
		auto bindingDescriptions = VreModel::Vertex::getBindingDescriptions();
		auto attributeDescriptions = VreModel::Vertex::getAttributeDescriptions();
		...
		if (vkCreateGraphicsPipelines(vreDevice.device(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &graphicsPipeline) != VK_SUCCESS) {
