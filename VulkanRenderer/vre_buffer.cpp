...
namespace vre {
    ...
    VreBuffer::~VreBuffer() {
        unmap();
        vkDestroyBuffer(vreDevice.device(), buffer, nullptr);
        vkFreeMemory(vreDevice.device(), memory, nullptr);
    }
    ...
    VkResult VreBuffer::map(VkDeviceSize size, VkDeviceSize offset) {
        ...
        return vkMapMemory(vreDevice.device(), memory, offset, size, 0, &mapped);
    }
    ...
    void VreBuffer::unmap() {
        if (mapped) {
            vkUnmapMemory(vreDevice.device(), memory);
            mapped = nullptr;
        }
    }
    ...
    VkResult VreBuffer::flush(VkDeviceSize size, VkDeviceSize offset) {
        ...
        return vkFlushMappedMemoryRanges(vreDevice.device(), 1, &mappedRange);
    }
    ...
    VkResult VreBuffer::invalidate(VkDeviceSize size, VkDeviceSize offset) {
        ...
        mappedRange.offset = offset;
        mappedRange.size = size;
        return vkInvalidateMappedMemoryRanges(vreDevice.device(), 1, &mappedRange);
    }
     ...
     *
     */
    VkResult VreBuffer::flushIndex(int index) { return flush(alignmentSize, index * alignmentSize); }
